...
--
-- Host: 127.0.0.1
-- Generation Time: 17-Jan-2019 às 19:11
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AtualizaEstoque_ap` (`id_prod_ap` INT, `qtde_comprada_ap` INT, `valor_unit_ap` DECIMAL(9,2))  BEGIN
    declare contador int(11);

    SELECT count(*) into contador FROM estoque_ap WHERE id_produto_ap = id_prod_ap;

    IF contador > 0 THEN
        UPDATE estoque_ap SET qtde_ap=qtde_ap + qtde_comprada_ap, valor_unitario_ap= valor_unit_ap
        WHERE id_produto_ap = id_prod_ap;
    ELSE
        INSERT INTO estoque_ap (id_produto_ap, qtde_ap, valor_unitario_ap) values (id_prod_ap, qtde_comprada_ap, valor_unit_ap);
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AtualizaEstoque_ee` (`id_prod_ee` INT, `qtde_comprada_ee` INT, `valor_unit_ee` DECIMAL(9,2))  BEGIN
    declare contador int(11);

    SELECT count(*) into contador FROM estoque_ee WHERE id_produto_ee = id_prod_ee;

    IF contador > 0 THEN
        UPDATE estoque_ee SET qtde_ee=qtde_ee + qtde_comprada_ee, valor_unitario_ee= valor_unit_ee
        WHERE id_produto_ee = id_prod_ee;
    ELSE
        INSERT INTO estoque_ee (id_produto_ee, qtde_ee, valor_unitario_ee) values (id_prod_ee, qtde_comprada_ee, valor_unit_ee);
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AtualizaEstoque_ie` (`id_prod_ie` INT, `qtde_comprada_ie` INT, `valor_unit_ie` DECIMAL(9,2))  BEGIN
    declare contador int(11);

    SELECT count(*) into contador FROM estoque_ie WHERE id_produto_ie = id_prod_ie;

    IF contador > 0 THEN
        UPDATE estoque_ie SET qtde_ie=qtde_ie + qtde_comprada_ie, valor_unitario_ie= valor_unit_ie
        WHERE id_produto_ie = id_prod_ie;
    ELSE
        INSERT INTO estoque_ie (id_produto_ie, qtde_ie, valor_unitario_ie) values (id_prod_ie, qtde_comprada_ie, valor_unit_ie);
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AtualizaEstoque_pp` (`id_prod_pp` INT, `qtde_comprada_pp` INT, `valor_unit_pp` DECIMAL(9,2))  BEGIN
    declare contador int(11);

    SELECT count(*) into contador FROM estoque_pp WHERE id_produto_pp = id_prod_pp;

    IF contador > 0 THEN
        UPDATE estoque_pp SET qtde_pp=qtde_pp + qtde_comprada_pp, valor_unitario_pp= valor_unit_pp
        WHERE id_produto_pp = id_prod_pp;
    ELSE
        INSERT INTO estoque_pp (id_produto_pp, qtde_pp, valor_unitario_pp) values (id_prod_pp, qtde_comprada_pp, valor_unit_pp);
    END IF;
END$$
...
--
-- Estrutura da tabela `cadcompras`
--

CREATE TABLE `cadcompras` (
  `idcompra` int(4) NOT NULL,
  `processo` varchar(24) NOT NULL,
  `datab` date NOT NULL,
  `finalidade` varchar(500) NOT NULL,
  `observacao` varchar(300) NOT NULL,
  `numreq` varchar(8) NOT NULL,
  `numcompra` varchar(10) NOT NULL,
  `pi` varchar(15) NOT NULL,
  `pi2` varchar(15) NOT NULL,
  `ptres` varchar(15) NOT NULL,
  `matserv` varchar(2) NOT NULL,
  `tipo` varchar(35) NOT NULL,
  `gerencia` varchar(5) NOT NULL,
  `nomereq` varchar(3) NOT NULL,
  `situacao` varchar(3) NOT NULL,
  `fase` varchar(4) NOT NULL,
  `dataval` varchar(12) NOT NULL,
  `finalcompra` int(1) NOT NULL,
...
CREATE TABLE `cadfornecedor` (
  ...
  `uf` varchar(3) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `email` varchar(50) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `fone` varchar(20) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `banco` varchar(25) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `agencia` varchar(10) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `conta` varchar(15) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
...
--
-- Estrutura da tabela `cadtipo`
--

CREATE TABLE `cadtipo` (
  `idtipo` int(4) NOT NULL,
  `tipo` varchar(50) NOT NULL,
  `texta` varchar(4000) NOT NULL,
  `textb` varchar(4000) NOT NULL,
  `textc` varchar(4000) NOT NULL,
  `textd` varchar(4000) NOT NULL,
  `texte` text NOT NULL,
  `textf` text NOT NULL,
  `textg` text NOT NULL,
  `texth` text NOT NULL,
...
--
-- Extraindo dados da tabela `cadtipo`
--

INSERT INTO `cadtipo` (`idtipo`, `tipo`, `texta`, `textb`, `textc`, `textd`, `texte`, `textf`, `textg`, `texth`, `texti`, `textj`) VALUES
(1, 'Pregão Eletrônico SRP', 'Considerando os documentos acostados no processo e tendo em vista que os itens definidos na REQUISIÇÃO são caracterizados como bens comuns e, considerando que o objeto pretendido se enquadra no § 1º do art. 2º do decreto 5.450 de 31 de maio de 2005.', 'Considerando a JUSTIFICATIVA, faz-se aos possíveis fornecedores, a oportunidade de apresentarem suas propostas de forma a promover a competitividade entre eles, bem como buscar o melhor preço e as melhores condições para a administração pública.', 'Em tudo de acordo com a  com a Lei nº 10.520 de 17.07.2002 e suas alterações, com o Decreto nº 3.555/2000 e suas alterações, com o Decreto nº 5.450/2005 e suas eventuais alterações e com aplicação subsidiária da Lei nº 8.666/93 e sua alteração.', 'Consoante a isso, conforme DECRETO Nº 7.892, DE 23 DE JANEIRO DE 2013, Art. 3º  O Sistema de Registro de Preços poderá ser adotado, sendo que para nossa necessidade atual ele se enquadra na(s) seguinte(s) hipótese(s):', '(  ) I - quando, pelas características do bem ou serviço, houver necessidade de contratações frequentes;', '(  ) II - quando for conveniente a aquisição de bens com previsão de entregas parceladas ou contratação de serviços remunerados por unidade de medida ou em regime de tarefa;', '(  ) III - quando for conveniente a aquisição de bens ou a contratação de serviços para atendimento a mais de um órgão ou entidade, ou a programas de governo; ou', '(  ) IV - quando, pela natureza do objeto, não for possível definir previamente o quantitativo a ser demandado pela Administração.', 'Após as considerações acima, Autorizo o prosseguimento da compra por PREGÃO ELETRÔNICO e pelo SISTEMA DE REGISTRO DE PREÇOS.', ''),
(2, 'Pregão Presencial', '', '', '', '', '', '', '', '', '', ''),
(3, 'Dispensa Incisos I e II', 'Considerando o Requerimento e a Justificativa apresentada.\r\nEm face de necessidade pública apresentada e especificada nos documentos acima elencados e a existência de bens ou serviços disponíveis (comprovados pelos orçamentos anexos) em tudo observadas as razões de ordem superior, relacionadas à satisfação de interesse público, também merecedor de imediata acolhida.\r\nConsiderando ainda que os benefícios de um eventual processo licitatório apresentar-se-iam inferiores aos malefícios dela derivados.\r\nEm razão do valor com base no artigo 24, inciso I e II da Lei 8.666/93.\r\nEm razão da compra ser realizada dentre orçamentos de vários fornecedores sem preferência ou vantagem dada a qualquer destes.', 'Autorizo a abertura da compra por Dispensa de licitação.', '', '', '', '', '', '', '', ''),
(4, 'Inexigibilidade', 'Considerando o disposto no artigo 25, caput, da lei 8.666/93, que preconiza que o órgão público deverá valer-se de processo de inexigibilidade de licitação quando houver inviabilidade de competição. \r\nConsiderando a justificativa acostada ao processo;', 'Autorizo a abertura do presente processo como inexigibilidade de licitação, devendo o mesmo ser encaminhado à Procuradoria Jurídica para análise de direito.', '', '', '', '', '', '', '', ''),
(5, 'Adesão de Ata', 'Tendo em vista a justificativa apresentada, bem como a disponibilização orçamentária para a referida aquisição.', 'Tendo sido analisada a previsão legal pelo Decreto 7892 , Art. 22 de \"Desde que devidamente justificada a vantagem, a ata de registro de preços, durante sua vigência, poderá ser utilizada por qualquer órgão ou entidade da administração pública federal que não tenha participado do certame licitatório, mediante anuência do órgão gerenciador\". ', 'Tendo sido verificada compras de outros órgãos da União de itens semelhantes e evidenciada a vantajosidade de aderirmos a(s) presente(s )ata(s) de registro de preços, mantendo-se os preços satisfatoriamente conforme os de mercado.', 'Tendo sido autorizada a adesão pelo órgão Gerenciador e dos fornecedores conforme instrução do processo.', ' Autoriza-se a aquisição dos bens do processo através da ADESÃO DE ATA DE REGISTRO DE PREÇOS.', '', '', '', '', ''),
(6, 'Adesão de IRP', ' Tendo em vista que os itens definidos na REQUISIÇÃO são caracterizados como bens comuns e, considerando que o objeto pretendido se enquadra no § 1º do art. 2º do decreto 5.450 de 31 de maio de 2005;\r\n', 'Considerando a JUSTIFICATIVA, faz-se aos possíveis fornecedores, a oportunidade de apresentarem suas propostas de forma a promover a competitividade entre eles, bem como buscar o melhor preço e as melhores condições para a administração pública; ', 'Autorizo a a adesão ao Intenção de Registro de Preços na modalidade PREGÃO ELETRÔNICO como adequada ao objeto do certame.', 'Em tudo de acordo com a  com a Lei nº 10.520 de 17.07.2002 e suas alterações, com o Decreto nº 3.555/2000 e suas alterações, com o Decreto nº 5.450/2005 e suas eventuais alterações, com o Decreto nº 7.892/2013 e suas eventuais alterações e com aplicação subsidiária da Lei nº 8.666/93 e sua alteração.', '', '', '', '', '', ''),
(7, 'Dispensa- Cotação Eletrônica', '', '', '', '', '', '', '', '', '', ''),
(8, 'Pregão Eletrônico', '', '', '', '', '', '', '', '', '', ''),
(9, 'Dispensa Inciso Demais', '', '', '', '', '', '', '', '', '', ''),
(10, 'Dispensa Inciso IV', 'Considerando o Requerimento e a Justificativa apresentada; \r\nEm face da situação emergencial levantada;\r\nEm atendimento ao artigo 24 da Lei 8.666/93.\r\nEm função dos prejuízos maiores a instituição caso não agirmos em tempo hábil.', 'Autorizo a abertura da compra por Dispensa de licitação, conforme Inciso IV, do Art. 24, da Lei 8.666/93.', '', '', '', '', '', '', '', '');

-- --------------------------------------------------------

--
-- Estrutura da tabela `cdbanco`
--

CREATE TABLE `cdbanco` (
...
CREATE TABLE `cdbem` (
  ...
  `patrimonio` int(15) NOT NULL,
  `ativo` varchar(3) NOT NULL,
  `disponivel` int(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='cad bens reservaveis';

--
-- Extraindo dados da tabela `cdbem`
--

INSERT INTO `cdbem` (`id`, `nome`, `atributo`, `patrimonio`, `ativo`, `disponivel`) VALUES
(1, 'fore disat', 'brano, axul e cores', 2147483647, 'Sim', 1);

-- --------------------------------------------------------

--
-- Estrutura da tabela `cdcontrole`
--

CREATE TABLE `cdcontrole` (
  `idc` int(5) NOT NULL,
  `tipo` varchar(50) NOT NULL,
...
--
-- Estrutura da tabela `cded`
--

CREATE TABLE `cded` (
  `ided` int(5) NOT NULL,
  `coded` varchar(15) NOT NULL,
  `nome` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Extraindo dados da tabela `cded`
--

INSERT INTO `cded` (`ided`, `coded`, `nome`) VALUES
(1, '3390.30.01', 'Combustiveis e lubrificantes automotivos '),
(2, '3390.30.03', 'Combustiveis e lubrif. P/ outras finalidades '),
(3, '3390.30.04', 'Gas e outros materiais engarrafados '),
(4, '3390.30.06', 'Alimentos para animais '),
(5, '3390.30.07', 'Generos de alimentacao '),
(6, '3390.30.08', 'Animais para pesquisa e abate '),
(7, '3390.30.09', 'Material farmacologico '),
(8, '3390.30.10', 'Material odontologico'),
(9, '3390.30.11', 'Material quimico'),
(10, '3390.30.12', 'Material de coudelaria ou de uso zootecnico'),
(11, '3390.30.13', 'Material de caca e pesca'),
(12, '3390.30.14', 'Material educativo e esportivo'),
(13, '3390.30.15', 'Material p/ festividades e homenagens'),
(14, '3390.30.16', 'Material de expediente'),
(15, '3390.30.17', 'Material de processamento de dados'),
(16, '3390.30.18', 'Materiais e medicamentos p/ uso veterinario'),
(17, '3390.30.19', 'Material de acondicionamento e embalagem'),
(18, '3390.30.20', 'Material de cama, mesa e banho'),
(19, '3390.30.21', 'Material de copa e cozinha'),
(20, '3390.30.22', 'Material de limpeza e prod. De higienizacao'),
(21, '3390.30.23', 'Uniformes, tecidos e aviamentos'),
(22, '3390.30.24', 'Material p/ manut.de bens imoveis/instalacoes'),
(23, '3390.30.25', 'Material p/ manutencao de bens moveis'),
(24, '3390.30.26', 'Material eletrico e eletronico'),
(25, '3390.30.27', 'Material de manobra e patrulhamento'),
(26, '3390.30.28', 'Material de protecao e seguranca'),
(27, '3390.30.29', 'Material p/ audio, video e foto'),
(28, '3390.30.30', 'Material para comunicacoes'),
(29, '3390.30.31', 'Sementes, mudas de plantas e insumos'),
(30, '3390.30.33', 'Material p/ producao industrial'),
(31, '3390.30.35', 'Material laboratorial'),
(32, '3390.30.36', 'Material hospitalar'),
(33, '3390.30.37', 'Sobressalentes de armamento'),
(34, '3390.30.38', 'Suprimento de protecao ao voo'),
(35, '3390.30.39', 'Material p/ manutencao de veiculos'),
(36, '3390.30.40', 'Material biologico'),
(37, '3390.30.41', 'Material p/ utilizacao em grafica'),
(38, '3390.30.42', 'Ferramentas'),
(39, '3390.30.43', 'Material p/ reabilitacao profissional'),
(40, '3390.30.44', 'Material de sinalizacao visual e outros'),
(41, '3390.30.45', 'Material tecnico p/ selecao e treinamento'),
(42, '3390.30.46', 'Material bibliografico'),
(43, '3390.30.47', 'Aquisicao de softwares de base'),
(44, '3390.30.48', 'Bens moveis nao ativaveis'),
(45, '3390.30.49', 'Bilhetes de passagem'),
(46, '3390.30.50', 'Bandeiras, flamulas e insignias'),
(47, '3390.30.51', 'Discotecas e filmotecas nao imobilizavel'),
(48, '3390.30.52', 'Material de carater secreto ou reservado'),
(49, '3390.30.53', 'Material meteorologico'),
(50, '3390.30.54', 'Material p/manut.conserv.de estradas e vias'),
(51, '3390.30.55', 'Selos para controle fiscal'),
(52, '3390.30.99', 'Outros materiais de consumo'),
(53, '3390.37.00', 'Locacao de mao-de-obra'),
(54, '3390.37.01', 'Apoio administrativo, tecnico e operacional'),
(55, '3390.37.02', 'Limpeza e conservacao'),
(56, '3390.37.03', 'Vigilancia ostensiva'),
(57, '3390.37.04', 'Manutencao e conservacao de bens imoveis'),
(58, '3390.37.05', 'Servicos de copa e cozinha'),
(59, '3390.37.06', 'Manutencao e conservacao de bens moveis'),
(60, '3390.37.99', 'Outras locacoes de mao de obra'),
(61, '3390.39.00', 'Outros servicos de terceiros-pessoa juridica'),
(62, '3390.39.01', 'Assinaturas de periodicos e anuidades'),
(63, '3390.39.05', 'Servicos tecnicos profissionais'),
(64, '3390.39.08', 'Manutencao de software'),
(65, '3390.39.09', 'Armazenagem'),
(66, '3390.39.10', 'Locacao de imoveis'),
(67, '3390.39.11', 'Locacao de softwares'),
(68, '3390.39.12', 'Locacao de maquinas e equipamentos'),
(69, '3390.39.16', 'Manutencao e conserv. De bens imoveis'),
(70, '3390.39.17', 'Manut. E conserv. De maquinas e equipamentos'),
(71, '3390.39.19', 'Manutencao e conserv. De veiculos'),
(72, '3390.39.20', 'Manut.e cons.de b.moveis de outras naturezas'),
(73, '3390.39.21', 'Manutencao e conserv. De estradas e vias'),
(74, '3390.39.22', 'Exposicoes, congressos e conferencias'),
(75, '3390.39.23', 'Festividades e homenagens'),
(76, '3390.39.41', 'Fornecimento de alimentacao'),
(77, '3390.39.42', 'Servicos de carater secreto ou reservado'),
(78, '34490.51.00', 'Obras e instalações'),
(79, '4490.51.80', 'Estudos e projetos '),
(80, '4490.51.91', 'Obras em andamento '),
(81, '4490.51.92', 'Instalações '),
(82, '4490.51.96', 'Almoxarifado de obras '),
(83, '4490.51.99', 'Outras obras e instalações '),
(84, '4490.52.00', 'Equipamentos e material permanente'),
(85, '4490.52.04', 'Aparelhos de medicao e orientacao '),
(86, '4490.52.06', 'Aparelhos e equipamentos de comunicacao '),
(87, '4490.52.08', 'Apar.equip.utens.med.,Odont,labor.hospit. '),
(88, '4490.52.10', 'Aparelhos e equip. P/ esportes e diversoes '),
(89, '4490.52.12', 'Aparelhos e utensilios domesticos '),
(90, '4490.52.18', 'Colecoes e materiais bibliograficos '),
(91, '4490.52.19', 'Discotecas e filmotecas '),
(92, '4490.52.24', 'Equipamento de protecao, seguranca e socorro '),
(93, '4490.52.26', 'Instrumentos musicais e artisticos '),
(94, '4490.52.28', 'Maquinas e equipam. De natureza industrial '),
(95, '4490.52.30', 'Maquinas e equipamentos energeticos '),
(96, '4490.52.32', 'Maquinas e equipamentos graficos '),
(97, '4490.52.33', 'Equipamentos para audio, video e foto '),
(98, '4490.52.34', 'Maquinas, utensilios e equipamentos diversos '),
(99, '4490.52.35', 'Equipamentos de processamento de dados '),
(100, '4490.52.36', 'Maquinas, instalacoes e utens. De escritorio '),
(101, '4490.52.38', 'Maq., Ferramentas e utensilios de oficina '),
(102, '4490.52.39', 'Equip. E utensilios hidraulicos e eletricos '),
(103, '4490.52.40', 'Maquinas e equipamentos agric. E rodoviarios '),
(104, '4490.52.42', 'Mobiliario em geral '),
(105, '4490.52.44', 'Obras de arte e pecas para exposicao '),
(106, '4490.52.48', 'Veiculos diversos '),
(107, '4490.52.51', 'Pecas nao incorporaveis a imoveis '),
(108, '4490.52.52', 'Veiculos de tracao mecanica '),
(109, '4490.52.57', 'Acessorios para veiculos '),
(110, '4490.52.58', 'Equipamentos de mergulho e salvamento '),
(111, '4490.52.83', 'Equipamentos e sistema de prot.vig.ambiental '),
(112, '4490.52.87', 'Material de consumo de uso duradouro '),
(113, '4490.52.99', 'Outros materiais permanentes '),
(114, '3391.39.90', 'Serviços de Publicidade Legal'),
(115, '4490.52. 41', 'Equipamentos de TIC - Computadores'),
(116, '4490.52.35 ', 'Material de TIC - Permanente'),
(117, '3390.39.16', 'Manutenção e Conservação de Bens Imóveis'),
(118, 'teste um', '1111.11.11'),
(119, 'Teste dois', '2222.22.22'),
(120, '3333.33.33', 'tres'),
(121, '5555.55.55', 'cinco');

-- --------------------------------------------------------

--
-- Estrutura da tabela `cdfase`
--

CREATE TABLE `cdfase` (
  `idfase` int(4) NOT NULL,
  `fase` varchar(25) NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8;

--
-- Extraindo dados da tabela `cdfase`
--

INSERT INTO `cdfase` (`idfase`, `fase`) VALUES
(1, 'Corrente'),
(2, 'Intermediario'),
(3, 'Permanente');
...
--
-- Estrutura da tabela `cdfinalidade`
--

CREATE TABLE `cdfinalidade` (
  `id` int(11) NOT NULL,
  `final` varchar(40) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Extraindo dados da tabela `cdfinalidade`
--

INSERT INTO `cdfinalidade` (`id`, `final`) VALUES
(1, 'Materiais'),
(2, 'Serviços');
...
--
-- Estrutura da tabela `cdhistorico`
--

CREATE TABLE `cdhistorico` (
  `idh` int(11) NOT NULL,
...
--
-- Extraindo dados da tabela `cdhistorico`
--

INSERT INTO `cdhistorico` (`idh`, `historico`, `observacao`) VALUES
(1, 'Estorna quantidade solicitado empenho', '');

-- --------------------------------------------------------

--
-- Estrutura da tabela `cdorgao`
--

CREATE TABLE `cdorgao` (
  `idorgao` int(10) NOT NULL,
  `uasg` int(6) NOT NULL,
  ...
  `iniprocesso` varchar(10) NOT NULL,
  `orgao_principal` varchar(1) NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8;

--
-- Extraindo dados da tabela `cdorgao`
--

INSERT INTO `cdorgao` (`idorgao`, `uasg`, `nome`, `endereco`, `cidade`, `uf`, `email`, `fone`, `gestor`, `diretor`, `fonte`, `iniprocesso`, `orgao_principal`) VALUES
(1, 155848, 'Instituto Federal de Educação, Ciência e Tecnologia do Mato Grosso do Sul - Campus Dourados ', 'Rua Filinto Muller, 1790 - Bairro Canaã I', 'Dourados', 'MS', 'gabin.dr@ifms.edu.br', '67-3410-8500', 'Danilo Sanches Dantas', 'Carlos Vinicius da Silva Figueiredo', '8100000000', '', 'S');

-- --------------------------------------------------------

--
-- Estrutura da tabela `cdos`
--
...
--
-- Estrutura da tabela `cdptres`
--

CREATE TABLE `cdptres` (
  `idp` int(3) NOT NULL,
...
--
-- Extraindo dados da tabela `cdptres`
--

INSERT INTO `cdptres` (`idp`, `nome`, `tipo`) VALUES
(1, 'PI', 'PI'),
(2, 'PTRES', 'PTRES'),
(3, 'L20RLP0100N', 'PI'),
(4, 'L20RLP600N', 'PI'),
...
--
-- Estrutura da tabela `cdrequisitante`
--

CREATE TABLE `cdrequisitante` (
  `idr` int(4) NOT NULL,
...
--
-- Estrutura da tabela `cdservidor`
--

CREATE TABLE `cdservidor` (
  `ids` int(4) NOT NULL,
  `nome` varchar(100) NOT NULL,
...
--
-- Estrutura da tabela `cdsetor`
--

CREATE TABLE `cdsetor` (
...
--
-- Estrutura da tabela `cdsituacao`
--

CREATE TABLE `cdsituacao` (
  `idsit` int(4) NOT NULL,
  `situacao` varchar(25) CHARACTER SET latin1 NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8;

--
-- Extraindo dados da tabela `cdsituacao`
--

INSERT INTO `cdsituacao` (`idsit`, `situacao`) VALUES
(1, 'Ativo'),
(2, 'Inativo');
...
--
-- Extraindo dados da tabela `cdunidade`
--

INSERT INTO `cdunidade` (`idun`, `unidade`) VALUES
(1, 'cotas'),
(2, 'Caixa'),
(3, 'Frasco'),
(4, 'Galão'),
...
INSERT INTO `condicao` (`idc`, `condicao`) VALUES
(1, 'Não'),
(2, 'Sim');
...
CREATE TABLE `entrada_produto` (
  ...
  `fornecedor` varchar(120) CHARACTER SET latin1 NOT NULL,
  `data_entrada` varchar(12) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Estrutura da tabela `entrada_produto_ee`
--

CREATE TABLE `entrada_produto_ee` (
  `id_ee` int(11) NOT NULL,
  `id_produto_ee` int(11) DEFAULT NULL,
  `qtde_ee` int(11) DEFAULT NULL,
  `valor_unitario_ee` decimal(9,2) DEFAULT '0.00',
  `fornecedor_ee` varchar(120) CHARACTER SET latin1 NOT NULL,
  `data_entrada_ee` date DEFAULT NULL,
  `nempenho_ee` varchar(20) CHARACTER SET latin1 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Acionadores `entrada_produto_ee`
--
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AD_ee` AFTER DELETE ON `entrada_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (old.id_produto_ee, old.qtde_ee * -1, old.valor_unitario_ee);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AI_ee` AFTER INSERT ON `entrada_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (new.id_produto_ee, new.qtde_ee, new.valor_unitario_ee);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AU_ee` AFTER UPDATE ON `entrada_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (new.id_produto_ee, new.qtde_ee - old.qtde_ee, new.valor_unitario_ee);
END
$$
DELIMITER ;
...
--
-- Estrutura da tabela `entrada_produto_ie`
--

CREATE TABLE `entrada_produto_ie` (
  `id_ie` int(11) NOT NULL,
  `id_produto_ie` int(11) DEFAULT NULL,
  `qtde_ie` int(11) DEFAULT NULL,
  `valor_unitario_ie` decimal(9,2) DEFAULT '0.00',
  `data_entrada_ie` varchar(10) CHARACTER SET latin1 DEFAULT NULL,
  `nempenho` varchar(13) NOT NULL,
  `data_empenho_ie` varchar(14) NOT NULL,
  `historico` int(2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Acionadores `entrada_produto_ie`
--
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AD_ie` AFTER DELETE ON `entrada_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (old.id_produto_ie, old.qtde_ie * -1, old.valor_unitario_ie);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AI_ie` AFTER INSERT ON `entrada_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (new.id_produto_ie, new.qtde_ie, new.valor_unitario_ie);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_EntradaProduto_AU_ie` AFTER UPDATE ON `entrada_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (new.id_produto_ie, new.qtde_ie - old.qtde_ie, new.valor_unitario_ie);
END
$$
DELIMITER ;
...
--
-- Estrutura da tabela `estoque_ee`
--

CREATE TABLE `estoque_ee` (
  `id_ee` int(11) NOT NULL,
  `id_produto_ee` int(11) DEFAULT NULL,
  `qtde_ee` int(11) DEFAULT NULL,
  `valor_unitario_ee` decimal(9,2) DEFAULT '0.00'
...
--
-- Estrutura da tabela `estoque_ie`
--

CREATE TABLE `estoque_ie` (
  `id_ie` int(11) NOT NULL,
  `id_produto_ie` int(11) DEFAULT NULL,
  `qtde_ie` int(11) DEFAULT NULL,
  `valor_unitario_ie` decimal(9,2) DEFAULT '0.00'
...
--
-- Estrutura da tabela `param_docs`
--
...
--
-- Extraindo dados da tabela `param_docs`
--

INSERT INTO `param_docs` (`id`, `tipo`, `codigo`) VALUES
(1, 'requisicao', '28'),
(2, 'despacho', '1');
...
CREATE TABLE `produto` (
  `id` int(11) NOT NULL,
  `status` char(10) CHARACTER SET latin1 NOT NULL DEFAULT 'A',
  ...
  `nitem` int(5) NOT NULL,
  `un` varchar(4) NOT NULL,
  `idprocesso` varchar(5) NOT NULL,
  `p1` decimal(10,2) NOT NULL,
  `p2` decimal(10,2) NOT NULL,
  `p3` decimal(10,2) NOT NULL,
  `dp1` varchar(10) CHARACTER SET latin1 NOT NULL,
  `dp2` varchar(10) CHARACTER SET latin1 NOT NULL,
  `dp3` varchar(10) CHARACTER SET latin1 NOT NULL,
  `fcp1` varchar(50) CHARACTER SET latin1 NOT NULL,
  `fcp2` varchar(50) CHARACTER SET latin1 NOT NULL,
  `fcp3` varchar(50) CHARACTER SET latin1 NOT NULL,
  `fnp1` varchar(100) CHARACTER SET latin1 NOT NULL,
  `fnp2` varchar(100) CHARACTER SET latin1 NOT NULL,
  `fnp3` varchar(100) CHARACTER SET latin1 NOT NULL,
  `pe1` varchar(20) CHARACTER SET latin1 NOT NULL,
  `pe2` varchar(20) CHARACTER SET latin1 NOT NULL,
  `pe3` varchar(20) CHARACTER SET latin1 NOT NULL,
  `tot_estimado` decimal(10,2) NOT NULL,
  `tot_estimado2` decimal(10,2) NOT NULL,
  `tot_estimado3` decimal(10,2) NOT NULL,
  `ip` varchar(2) CHARACTER SET latin1 NOT NULL,
  `ed` varchar(5) CHARACTER SET latin1 NOT NULL,
  `grupo` int(4) NOT NULL,
  `valor_unitario` decimal(10,4) NOT NULL,
  `fornecedora` varchar(5) NOT NULL,
  `dataval` varchar(12) NOT NULL,
...
CREATE TABLE `saida_produto` (
  ...
  `id_produto` int(11) DEFAULT NULL,
  `qtde` int(11) DEFAULT NULL,
  `data_saida` varchar(10) CHARACTER SET latin1 DEFAULT NULL,
  `valor_unitario` decimal(9,2) DEFAULT '0.00',
  `nempenho` varchar(12) NOT NULL,
  `data_empenho` varchar(10) NOT NULL,
  `historico` int(2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Estrutura da tabela `saida_produto_ee`
--

CREATE TABLE `saida_produto_ee` (
  `id_ee` int(11) NOT NULL,
  `id_produto_ee` int(11) DEFAULT NULL,
  `qtde_ee` int(11) DEFAULT NULL,
  `data_saida_ee` varchar(13) DEFAULT NULL,
  `valor_unitario_ee` decimal(9,2) DEFAULT '0.00',
  `nempenho_ee` varchar(14) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Acionadores `saida_produto_ee`
--
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AD_ee` AFTER DELETE ON `saida_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (old.id_produto_ee, old.qtde_ee, old.valor_unitario_ee);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AI_ee` AFTER INSERT ON `saida_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (new.id_produto_ee, new.qtde_ee * -1, new.valor_unitario_ee);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AU_ee` AFTER UPDATE ON `saida_produto_ee` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ee (new.id_produto_ee, old.qtde_ee - new.qtde_ee, new.valor_unitario_ee);
END
$$
DELIMITER ;
...
--
-- Estrutura da tabela `saida_produto_ie`
--

CREATE TABLE `saida_produto_ie` (
  `id_ie` int(11) NOT NULL,
  `id_produto_ie` int(11) DEFAULT NULL,
  `qtde_ie` int(11) DEFAULT NULL,
  `data_saida_ie` varchar(12) DEFAULT NULL,
  `valor_unitario_ie` decimal(9,2) DEFAULT '0.00',
  `nempenho_ie` varchar(20) CHARACTER SET latin1 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Acionadores `saida_produto_ie`
--
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AD_ie` AFTER DELETE ON `saida_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (old.id_produto_ie, old.qtde_ie, old.valor_unitario_ie);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AI_ie` AFTER INSERT ON `saida_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (new.id_produto_ie, new.qtde_ie * -1, new.valor_unitario_ie);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `TRG_SaidaProduto_AU_ie` AFTER UPDATE ON `saida_produto_ie` FOR EACH ROW BEGIN
      CALL SP_AtualizaEstoque_ie (new.id_produto_ie, old.qtde_ie - new.qtde_ie, new.valor_unitario_ie);
END
$$
DELIMITER ;
...
CREATE TABLE `users` (
  ...
  `lotacao` varchar(200) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `admin` int(2) NOT NULL,
  `setor` varchar(50) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `email` varchar(100) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `senha` varchar(64) NOT NULL,
  `situacao` int(1) NOT NULL,
  `requisitante` int(2) NOT NULL,
  `comprador` int(2) NOT NULL,
  `diretor` int(2) NOT NULL,
  `almoxarife` int(2) NOT NULL,
  `boletim` int(1) NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
INSERT INTO `users` (`id`, `nome`, `lotacao`, `admin`, `setor`, `email`, `senha`, `situacao`, `requisitante`, `comprador`, `diretor`, `almoxarife`, `boletim`) VALUES
(3, 'Administrador', '', 2, '', 'adm', 'b09c600fddc573f117449b3723f23d64', 1, 2, 2, 2, 2, 0),
(6, 'Sedenir Deparis', '', 2, '', 'admin', 'b09c600fddc573f117449b3723f23d64', 1, 2, 2, 2, 2, 0),
(7, 'Sedenir Deparis', '', 2, '', 'sedenir', 'be2d8fce4c8126e60d35bd58018c457b', 1, 2, 2, 2, 2, 0);
...
INSERT INTO `versao` (`id`, `versao`, `data`, `css`) VALUES
(1, '1.0.3', '15/01/2019', 'Lemmis 2017 | Design by: www.binarytheme.com');
...
--
-- Indexes for table `cadcompras`
--
ALTER TABLE `cadcompras`
...
--
-- Indexes for table `cadtipo`
--
ALTER TABLE `cadtipo`
  ADD PRIMARY KEY (`idtipo`);

--
-- Indexes for table `cdbanco`
--
ALTER TABLE `cdbanco`
  ADD PRIMARY KEY (`idbanco`);

--
-- Indexes for table `cdbem`
--
ALTER TABLE `cdbem`
  ADD PRIMARY KEY (`id`);

--
-- Indexes for table `cdcontrole`
--
ALTER TABLE `cdcontrole`
  ADD PRIMARY KEY (`idc`);

--
-- Indexes for table `cded`
--
ALTER TABLE `cded`
  ADD PRIMARY KEY (`ided`);

--
-- Indexes for table `cdfase`
--
ALTER TABLE `cdfase`
  ADD PRIMARY KEY (`idfase`);

--
-- Indexes for table `cdfinalidade`
--
ALTER TABLE `cdfinalidade`
  ADD PRIMARY KEY (`id`);

--
-- Indexes for table `cdhistorico`
--
ALTER TABLE `cdhistorico`
  ADD PRIMARY KEY (`idh`);

--
-- Indexes for table `cdorgao`
--
ALTER TABLE `cdorgao`
  ADD PRIMARY KEY (`idorgao`);

--
-- Indexes for table `cdos`
--
ALTER TABLE `cdos`
  ADD PRIMARY KEY (`idos`);

--
-- Indexes for table `cdptres`
--
ALTER TABLE `cdptres`
  ADD PRIMARY KEY (`idp`);

--
-- Indexes for table `cdrequisitante`
--
ALTER TABLE `cdrequisitante`
  ADD PRIMARY KEY (`idr`);

--
-- Indexes for table `cdservidor`
--
ALTER TABLE `cdservidor`
  ADD PRIMARY KEY (`ids`);

--
-- Indexes for table `cdsetor`
--
ALTER TABLE `cdsetor`
  ADD PRIMARY KEY (`idsetor`);

--
-- Indexes for table `cdsituacao`
--
ALTER TABLE `cdsituacao`
  ADD PRIMARY KEY (`idsit`);

--
-- Indexes for table `cdunidade`
--
ALTER TABLE `cdunidade`
...
--
-- Indexes for table `entrada_produto_ee`
--
ALTER TABLE `entrada_produto_ee`
  ADD PRIMARY KEY (`id_ee`);

--
-- Indexes for table `entrada_produto_ie`
--
ALTER TABLE `entrada_produto_ie`
...
--
-- Indexes for table `estoque_ee`
--
ALTER TABLE `estoque_ee`
  ADD PRIMARY KEY (`id_ee`);

--
-- Indexes for table `estoque_ie`
--
ALTER TABLE `estoque_ie`
  ADD PRIMARY KEY (`id_ie`);

--
-- Indexes for table `param_docs`
--
ALTER TABLE `param_docs`
  ADD PRIMARY KEY (`id`);
...
--
-- Indexes for table `saida_produto_ee`
--
ALTER TABLE `saida_produto_ee`
  ADD PRIMARY KEY (`id_ee`);

--
-- Indexes for table `saida_produto_ie`
--
ALTER TABLE `saida_produto_ie`
  ADD PRIMARY KEY (`id_ie`);
...
--
-- AUTO_INCREMENT for table `cadcompras`
--
ALTER TABLE `cadcompras`
...
--
ALTER TABLE `cadfornecedor`
  MODIFY `idforn` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cadtipo`
--
ALTER TABLE `cadtipo`
  MODIFY `idtipo` int(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT for table `cdbanco`
--
ALTER TABLE `cdbanco`
  MODIFY `idbanco` int(3) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cdbem`
--
ALTER TABLE `cdbem`
  MODIFY `id` int(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT for table `cdcontrole`
--
ALTER TABLE `cdcontrole`
  MODIFY `idc` int(5) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cded`
--
ALTER TABLE `cded`
  MODIFY `ided` int(5) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=122;

--
-- AUTO_INCREMENT for table `cdfase`
--
ALTER TABLE `cdfase`
  MODIFY `idfase` int(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `cdfinalidade`
--
ALTER TABLE `cdfinalidade`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `cdhistorico`
--
ALTER TABLE `cdhistorico`
  MODIFY `idh` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT for table `cdorgao`
--
ALTER TABLE `cdorgao`
  MODIFY `idorgao` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT for table `cdos`
--
ALTER TABLE `cdos`
  MODIFY `idos` int(4) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cdptres`
--
ALTER TABLE `cdptres`
  MODIFY `idp` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT for table `cdrequisitante`
--
ALTER TABLE `cdrequisitante`
  MODIFY `idr` int(4) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cdservidor`
--
ALTER TABLE `cdservidor`
  MODIFY `ids` int(4) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cdsetor`
--
ALTER TABLE `cdsetor`
  MODIFY `idsetor` int(4) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `cdsituacao`
--
ALTER TABLE `cdsituacao`
  MODIFY `idsit` int(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `cdunidade`
--
ALTER TABLE `cdunidade`
  MODIFY `idun` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=39;

--
-- AUTO_INCREMENT for table `condicao`
--
ALTER TABLE `condicao`
  MODIFY `idc` int(2) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
-- AUTO_INCREMENT for table `entrada_produto_ee`
--
ALTER TABLE `entrada_produto_ee`
  MODIFY `id_ee` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `entrada_produto_ie`
--
ALTER TABLE `entrada_produto_ie`
  MODIFY `id_ie` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `estoque_ee`
--
ALTER TABLE `estoque_ee`
  MODIFY `id_ee` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `estoque_ie`
--
ALTER TABLE `estoque_ie`
  MODIFY `id_ie` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `param_docs`
--
ALTER TABLE `param_docs`
...
--
-- AUTO_INCREMENT for table `saida_produto_ee`
--
ALTER TABLE `saida_produto_ee`
  MODIFY `id_ee` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `saida_produto_ie`
--
ALTER TABLE `saida_produto_ie`
  MODIFY `id_ie` int(11) NOT NULL AUTO_INCREMENT;
